...
public class BoardDAO {
	...
	public void getBoardListWithPaging(){
		String BOARD_LIST2_WITH_PAGING = "SELECT * from ( SELECT ROWNUM AS row_num, board.* FROM ( SELECT * FROM board ORDER BY seq DESC ) board ) WHERE row_num >= ? AND row_num <= ?";
		String BOARD_LIST_T2_WITH_PAGING = "SELECT * from ( SELECT ROWNUM AS row_num, board.* FROM ( SELECT * FROM board WHERE TITLE LIKE '%'||?||'%' ORDER BY seq DESC ) board ) WHERE row_num >= ? AND row_num <= ?";
		String BOARD_LIST_C2_WITH_PAGING = "SELECT * from ( SELECT ROWNUM AS row_num, board.* FROM ( SELECT * FROM board WHERE CONTENT LIKE '%'||?||'%' ORDER BY seq DESC ) board ) WHERE row_num >= ? AND row_num <= ?";
		
	}//end getBoardListWithPaging()
	
	
	//게시물 수 관련 메소드
	public void getBoardCnt(){
		String BOARD_LIST2_CNT = "SELECT COUNT(*) FROM ( SELECT * FROM board ORDER BY seq DESC ) board";
		String BOARD_LIST_T2_CNT = "SELECT COUNT(*) from ( SELECT ROWNUM AS row_num, board.* FROM ( SELECT * FROM board ORDER BY seq DESC ) board ) WHERE TITLE LIKE '%'||?||'%'";
		String BOARD_LIST_C2_CNT = "SELECT COUNT(*) from ( SELECT ROWNUM AS row_num, board.* FROM ( SELECT * FROM board ORDER BY seq DESC ) board ) WHERE CONTENT LIKE '%'||?||'%'";
